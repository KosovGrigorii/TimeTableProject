...
namespace TimetableApplication
{
    public abstract class OutputFormatter
    {
        ...
        protected (Dictionary<string, string[,]>, List<Tuple<TimeSpan, TimeSpan>>) GetTables(List<TimeSlot> timeSlots)
        {
            var teachers = new Dictionary<Teacher, List<TimeSlot>>();
            var groups = new Dictionary<Group, List<TimeSlot>>();
            var bellStartSet = new HashSet<TimeSpan>();
            var bellEndSet = new HashSet<TimeSpan>();
            foreach (var timeSlot in timeSlots)
            {
                bellStartSet.Add(timeSlot.Start);
                bellEndSet.Add(timeSlot.End);
            ...
            var bells = bellStartSet
                .OrderBy(time => time.Milliseconds)
                .Zip(bellEndSet
                    .OrderBy(time => time.Milliseconds), Tuple.Create)
                .ToList();
